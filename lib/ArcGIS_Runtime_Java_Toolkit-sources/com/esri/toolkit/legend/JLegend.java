 ...
 * contentPane.add(legend, BorderLayout.WEST); // adds legend to a component with border layout
 * </pre>
 * </code>
 * @since 10.2.3
 */
public class JLegend extends JPanel {
  ...
  protected class LegendLayerListEventListener extends LayerListEventListenerAdapter {
    ...
    public void multipleLayersAdded(LayerEvent event) {
      for (Map.Entry<Integer, Layer> curEntry : event.getChangedLayers().entrySet()) {
        addLayerToLegend(curEntry.getValue(), _rootNode, curEntry.getKey().intValue());
      }
   ...
   * <p/>
   * If the user object is null, then the expanding node represents either a layer or a sublayer. For layers, sublayer
   * information will be added if there is any, otherwise legend information is added. For sublayers, either more
   * sublayers or the sublayer's legend information will be added.
   * <p/>
   ...
   * will have a dummy child node added that has the actual legend item collection as a user object. This will allow
